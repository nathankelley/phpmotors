...
// THIS IS THE IMAGE UPLOADS PHP MOTORS MODEL
...
// Get thumbnail image path from images table
function getThumbnailFromImages($invId){
    $db = phpmotorsConnect();
    $sql = 'SELECT * from images 
    join inventory on images.invId = inventory.invId 
    where imgPath like "%-tn%" and images.invId = :invId';
    $stmt = $db->prepare($sql);
    $stmt->bindValue(':invId', $invId, PDO::PARAM_INT);
    $stmt->execute();
    $imageMatch = $stmt->fetchAll(PDO::FETCH_ASSOC);
    $stmt->closeCursor();
    return $imageMatch;
}
?>